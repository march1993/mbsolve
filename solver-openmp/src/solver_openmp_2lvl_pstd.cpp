...
#include <cmath>
#include <omp.h>
#include <solver_openmp_2lvl_pstd.hpp>
#include <common_openmp.hpp>

namespace mbsolve {

static solver_factory<solver_openmp_2lvl_pstd> factory("openmp-2lvl-pstd");

solver_openmp_2lvl_pstd::solver_openmp_2lvl_pstd
...
solver_int(dev, scen)
{
    ...
    m_buf_fft = fftw_alloc_complex(num_gridpoints);
...
const std::string&
solver_openmp_2lvl_pstd::get_name() const
{
    return factory.get_name();
}

void
solver_openmp_2lvl_pstd::run() const
{
    unsigned int num_gridpoints = m_scenario->get_num_gridpoints();
    unsigned int num_timesteps = m_scenario->get_num_timesteps();
    unsigned int num_sources = m_sim_sources.size();
